...
// ==加载Lodop对象的主过程:==
(function loadCLodop() {
    if (!window.WebSocket && (window as any).MozWebSocket) (window as any).WebSocket = (window as any).MozWebSocket;
    // ws方式速度快(小于200ms)且可避免CORS错误,但要求Lodop版本足够新:
    try {
        const WSK1 = new WebSocket(URL_WS1);
        WSK1.onopen = function (e) { setTimeout(checkOrTryHttp, 200); };
        WSK1.onmessage = function (e) { if (!(window as any).getCLodop) eval(e.data); };
        WSK1.onerror = function (e) {
            const WSK2 = new WebSocket(URL_WS2);
            WSK2.onopen = function (e) { setTimeout(checkOrTryHttp, 200); };
            WSK2.onmessage = function (e) { if (!(window as any).getCLodop) eval(e.data); };
...
// ==获取LODOP对象的主过程:==
export default function getLodop(oOBJECT?: HTMLElement | null, oEMBED?: HTMLElement | null): any {
    let LODOP: any;
    try {
